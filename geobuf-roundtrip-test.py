...
def system(cmd: str, *, verbose: bool = False, dry_run: bool = False, assert_return: bool = True):
    if dry_run:
        print(f'$ {cmd}')
        return
    ret = os.system(cmd)
...
def roundtrip_cpp(input_json: str, output_pbf: str, output_json: str):
    system(f'{PWD}/build/bin/json2geobuf {input_json} > {output_pbf}', dry_run=False)
    system(f'{PWD}/build/bin/geobuf2json {output_pbf} > {output_json}', dry_run=False)
...
def roundtrip(input_json: str, *, force_rerun: bool = False,):
    ...
    system(f'diff {output_py} {output_cpp} > {output_dir}/diff_py_cpp.diff', assert_return=False)
